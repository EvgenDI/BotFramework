...
namespace Bot_Application1
{
    ...
    public class SmsRu
    {
        private const string phoneRegExPattern = @"^(\s*)?(\+)?([- _():=+]?\d[- _():=+]?){11,14}(\s*)?$";
        ...
        public bool CorrectPhone(string number)
        {
            if (Regex.IsMatch(number, phoneRegExPattern))
            {
                return true;
            }
            else
        ...
        public string DeleteSymbol(string text)
        {
